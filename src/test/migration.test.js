...
describe('migrateState', () => {
  ...
  it('adds default leave types when missing', () => {
    ...
    expect(migrateState(state).settings.leaveTypes).toEqual(defaultLeaveTypes);
  });
  ...
  it('sets type to semester for migrated blocks', () => {
    ...
    expect(migrateState(state).vacationBlocks[0].type).toBe('semester');
  });
  ...
  it('adds empty comment to migrated blocks', () => {
    ...
    expect(migrateState(state).vacationBlocks[0].comment).toBe('');
  });
